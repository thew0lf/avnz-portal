...
export class JiraForceController {
  ...
  async forceStart(@Req() req: any, @Res() res: Response) {
    ...
    if (keys.some(key => typeof key !== 'string')) throw new BadRequestException('All keys must be strings.');
    ...
    if (req.query.format === 'csv') {
      if (results.length === 0) return res.send(''); // Return empty CSV instead of throwing an exception
