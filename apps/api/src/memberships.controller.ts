...
export class MembershipsController {
  ...
  async list(@Req() req: Request & { auth?: any }) {
    ...
    try {
      ...
      if (!clientId) {
        ...
        const { rows } = await client.query(
          `select m.user_id, m.role, u.email, u.username, m.created_at
            ...
            where m.org_id=$1
            order by coalesce(u.email,u.username) asc`,
          [org]
        )
        return { rows }
      }
      ...
      if (!perms.includes('manage_members') && !perms.includes('admin')) {
        ...
        if (!perms.includes('manage_clients')) throw new ForbiddenException('forbidden')
        ...
        if (!chk.rows[0]) throw new ForbiddenException('forbidden')
      }
      const { rows } = await client.query(
        `select cm.user_id, 'client'::text as role, u.email, u.username, cm.created_at
  ...
  async add(@Req() req: Request & { auth?: any }, @Body() body: any) {
    ...
    try {
      ...
      await audit(req as any, 'upsert', 'membership', String(user.id), null, { org_id: org, role, role_id: rid })
      return { ok: true }
    } finally { client.release() }
  }
}
