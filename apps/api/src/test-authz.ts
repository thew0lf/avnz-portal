...
async function run(){
  ...
  try {
    ...
    const org = randomUUID(); const client = randomUUID(); const company = randomUUID(); const dept = randomUUID(); const team = randomUUID(); const group = randomUUID();
    ...
    // roles and actions
    const roleCompanyAdmin = randomUUID(); const roleTeamOwner = randomUUID();
    ...
    // permissions
    const perm1 = randomUUID(); const perm2 = randomUUID();
    ...
    if (!allow1.allowed || allow2.allowed) { console.error('Authz test failed'); process.exit(1) }
    ...
    if (abacFail.allowed || !abacPass.allowed) { console.error('ABAC test failed'); process.exit(1) }

    // Cache invalidation via NOTIFY: lower requirement for manage_roster from TeamOwner(10) to level 0 new role
    const roleAny = randomUUID(); await c.query('insert into authz.roles(id,name,level) values ($1,$2,$3) on conflict do nothing',[roleAny,'Any',0])
    // prime cache
    const cachedDeny = await svc.isAllowed(user, team, 'node','team','manage_roster')
    if (cachedDeny.allowed) { console.error('Unexpected allow before change'); process.exit(1) }
    // start listener
    ...
    if (!nowAllow.allowed) { console.error('Notify invalidation failed'); process.exit(1) }
    console.log('Authz tests passed')
  } catch(e){ console.error(e); try{ await c.query('rollback') }catch{} process.exit(1) } finally { c.release() }
}
