...
export class SpecController {
  ...
  async invite(@Body() body:any, req?: any){
    ...
    const c = await pool.connect(); try {
      ...
      if (contact.email) { try { await sendInviteEmail(contact.email, token, { clientName: clientNode.name, clientId, orgId }) } catch(e) { console.warn('email fail',e) } }
      if (contact.phone) { try { await sendInviteSms(contact.phone, token, { shortCode: code, clientName: clientNode.name, clientId, orgId }) } catch (e) { console.warn('sms fail', e) } }
  ...
  @Get('app-settings')
  async appSettings(){ const c=await pool.connect(); try{ const r=await c.query("select key,value from app_settings"); const obj:any={}; for(const row of r.rows){ obj[row.key]=row.value } return obj } finally { c.release() } }

  // Client-scoped secrets (read-only) for users with view_client_secrets + membership
  @Get('clients/:clientId/services/configs')
  async listClientConfigs(@Param('clientId') clientId:string, @Req() req: any){
    const ctx = req.auth || {}; const userId = ctx.userId; if (!userId) throw new ForbiddenException('unauthorized')
    const c=await pool.connect(); try{
      ...
      if (!m.rows[0]) throw new ForbiddenException('membership required')
      const perms: string[] = Array.isArray((ctx as any).perms)? (ctx as any).perms: []
      if (!perms.includes('view_client_secrets') && !perms.includes('admin')) throw new ForbiddenException('view_client_secrets required')
