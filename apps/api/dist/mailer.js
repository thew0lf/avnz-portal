...
export async function sendRawEmail(to, subject, text, html, options) {
    const cfg = getConfig();
    const dbFrom = options?.orgId ? await getServiceConfig(options.orgId, options?.clientId || null, 'sendgrid', 'from') : null;
    if (dbFrom)
        cfg.from = dbFrom;
        ...
        try {
            ...
            try {
                ...
                if (orgId) {
                    ...
                    try {
                        await cU.query('insert into usage_events(org_id, provider, model, operation, embed_tokens, cost_usd) values ($1,$2,$3,$4,$5,$6)', [orgId, 'sendgrid', 'email', 'send', 1000, cost]);
                    }
                    finally {
                        cU.release();
                    }
    ...
    const t = getTransporter();
    if (!t) {
        console.log('Preview send (no mail transport):', { to, subject });
        return;
    }
    await t.sendMail({ from: cfg.from, to, subject, text, html });
    try {
        ...
        if (orgId) {
            ...
            const c = await pool.connect();
            try {
                await c.query('insert into usage_events(org_id, provider, model, operation, embed_tokens, cost_usd) values ($1,$2,$3,$4,$5,$6)', [orgId, 'smtp', 'email', 'send', 1000, cost]);
            }
            finally {
                c.release();
            }
