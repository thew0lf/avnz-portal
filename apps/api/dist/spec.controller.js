...
function genShort(len = 6) { const a = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; let s = ''; for (let i = 0; i < len; i++)
    s += a[Math.floor(Math.random() * a.length)]; return s; }
let SpecController = class SpecController {
    // First-run bootstrap: create first org + owner
    async bootstrapOrg(body, req) {
        const { org_name, admin_email, password } = body || {};
        if (!org_name || !admin_email || !password)
            throw new BadRequestException('org_name, admin_email, password required');
        ...
        try {
            ...
            if (!roleId)
                throw new Error('missing OrgOwner role seed');
            ...
            await audit(req, 'create', 'bootstrap.org', orgNodeId, null, { org_name, admin_email });
    ...
    async createClient(body, req) {
        ...
        if (!orgNodeId || !name)
            throw new BadRequestException('orgNodeId, name required');
        const c = await pool.connect();
        try {
            // find parent path
            const p = await c.query('select path::text as path from authz.nodes where id=$1 and type=$2', [orgNodeId, 'org']);
            const parentPath = p.rows[0]?.path;
            if (!parentPath)
                throw new BadRequestException('invalid orgNodeId');
            // generate unique code
            let code = null;
            for (let i = 0; i < 10; i++) {
                const candidate = genShort().toLowerCase();
                const exists = await c.query('select 1 from clients where code=$1 limit 1', [candidate]);
                if (!exists.rows[0]) {
                    code = candidate;
                    break;
                }
            ...
            if (!code)
                throw new BadRequestException('failed to generate short code');
            await c.query('begin');
            // create client node
            ...
            try {
                await c.query('insert into clients(org_id, code, name) values ($1,$2,$3)', [orgNodeId, code, name]);
            }
            catch { }
            await c.query('commit');
            await audit(req, 'create', 'client', nodeId, null, { name, code });
    ...
    async getClient(id) { const c = await pool.connect(); try {
        const n = await c.query("select id,name from authz.nodes where id=$1 and type='client'", [id]);
        const node = n.rows[0];
        if (!node)
            throw new BadRequestException('not found');
        const cc = await c.query('select code from clients where org_id=$1 limit 1', [id]);
        return { id, name: node.name, clientShortCode: cc.rows[0]?.code || null };
    }
    finally {
        c.release();
    } }
    ...
    async invite(body, req) {
        ...
        if (!clientId || !contact || (!contact.email && !contact.phone))
            throw new BadRequestException('clientId and contact.email|phone required');
        const c = await pool.connect();
        try {
            const cl = await c.query('select id,name from authz.nodes where id=$1 and type=$2', [clientId, 'client']);
    ...
    async acceptViaPath(short, token, body, req) {
        const { username, password } = body || {};
        if (!password)
            throw new BadRequestException('password required');
        const c = await pool.connect();
        try {
            const cl = await c.query('select id from clients where code=$1', [String(short).toLowerCase()]);
            ...
            if (inv.revoked || inv.used_at || new Date(inv.expires_at).getTime() < Date.now())
                throw new BadRequestException('invite not valid');
            await validatePassword(c, String(password));
            const pw = await hashPassword(String(password));
            ...
            await audit(req, 'update', 'invite.accept', inv.id, null, { user_id: user.id });
            return { ok: true };
        }
        finally {
            c.release();
    ...
    async assignRole(body) { const { userId, nodeId, roleId } = body || {}; if (!userId || !nodeId || !roleId)
        throw new BadRequestException('missing'); const c = await pool.connect(); try {
        const r = await c.query('insert into authz.role_assignments(id,user_id,node_id,role_id) values (gen_random_uuid(),$1,$2,$3) returning id', [userId, nodeId, roleId]);
        return r.rows[0];
    }
    finally {
        c.release();
...
], SpecController.prototype, "bootstrapOrg", null);
__decorate([
    Post('clients'),
    __param(0, Body()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], SpecController.prototype, "createClient", null);
__decorate([
    Get('clients/:id'),
    __param(0, Param('id')),
    __metadata("design:type", Function),
    ...
    __metadata("design:returntype", Promise)
], SpecController.prototype, "getClient", null);
__decorate([
    Post('invitations'),
    __param(0, Body()),
    __metadata("design:type", Function),
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], SpecController.prototype, "acceptViaPath", null);
__decorate([
    Get('capabilities/me'),
    __param(0, Req()),
    __metadata("design:type", Function),
    ...
    __metadata("design:returntype", Promise)
], SpecController.prototype, "capsMe", null);
__decorate([
    Post('roles/assign'),
    __param(0, Body()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SpecController.prototype, "assignRole", null);
__decorate([
    ...
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], SpecController.prototype, "revokeRole", null);
SpecController = __decorate([
    Controller('api')
], SpecController);
