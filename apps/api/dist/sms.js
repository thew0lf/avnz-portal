...
export async function sendInviteSms(to, token, opts) {
    ...
    if (!sid || !auth || !from) {
        console.warn('SMS disabled: missing Twilio env');
        return;
    }
    ...
    const vars = { url, clientName, shortCode: opts?.shortCode || '' };
    ...
    try {
        if (opts?.orgId) {
            const unit = await unitPrice('twilio', 'sms', 'embed_tokens', opts.orgId, undefined, []);
            const cost = unit * (1000 / 1000);
            const c2 = await pool.connect();
            try {
                await c2.query('insert into usage_events(org_id, provider, model, operation, embed_tokens, cost_usd) values ($1,$2,$3,$4,$5,$6)', [opts.orgId, 'twilio', 'sms', 'send', 1000, cost]);
            }
            finally {
                c2.release();
            }
