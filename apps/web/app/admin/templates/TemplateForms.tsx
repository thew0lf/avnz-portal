  ...
  return (
    <div className="space-y-4">
      <Section title="Email Templates">
        <form onSubmit={upsertEmail} ref={emailFormRef} className="grid gap-4">
          <div>
            <label className="block text-sm text-muted-foreground">Key</label>
          ...
          <div>
            ...
            <EmailMjmlEditor value={mjml} onChange={setMjml} />
          </div>
          ...
          <TableBody>
            {emailTemplates.map((t:any)=> (
              <TableRow key={t.id}>
                ...
                <TableCell>{keyToName[t.client_id] || 'Org default'}</TableCell>
                <TableCell>{new Date(t.updated_at).toLocaleString('en-US',{ timeZone:'UTC' })}</TableCell>
                <TableCell>
                  <ActionButton variant="secondary" label="Edit" method="GET" path={`/admin/templates/email/${encodeURIComponent(t.id)}`} />
                  <ActionButton variant="secondary" label="Delete" method="DELETE" path={`/admin/templates/email/${encodeURIComponent(t.id)}`} />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
