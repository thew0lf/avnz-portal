...
export function DashboardUsageChart(){
  ...
  async function load(){
    ...
    try{
      ...
      const data = await r.json(); const rows: any[] = (data.rows||[]).map((d:any)=>({ date: new Date(d.bucket).toLocaleDateString(undefined,{ month:'short', day:'2-digit' }), cost: Number(d.cost_usd||0) })); setRows(rows as any)
