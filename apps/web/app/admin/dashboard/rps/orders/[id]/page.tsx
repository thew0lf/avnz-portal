...
async function load(id: string){
  const r = await fetch(absoluteUrl(`/api/rps/orders/${encodeURIComponent(id)}`), { cache: 'no-store' })
  if (!r.ok) return null
  return r.json()
}
  ...
  return (
    <div className="p-6 grid gap-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Created {o.created_at? new Date(o.created_at).toLocaleString() : '—'}</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="border rounded p-3">
          ...
          {data.billingAddress ? (
            <div className="text-sm whitespace-pre-line">{[data.billingAddress.name, data.billingAddress.line1, data.billingAddress.line2, `${data.billingAddress.city||''} ${data.billingAddress.region||data.billingAddress.state||''} ${data.billingAddress.postal_code||''}`, data.billingAddress.country].filter(Boolean).join('\n')}</div>
          ) : (<div className="text-sm text-muted-foreground">—</div>)}
        </div>
        <div className="border rounded p-3">
          <h2 className="font-medium mb-2">Shipping Address</h2>
          {data.shippingAddress ? (
            <div className="text-sm whitespace-pre-line">{[data.shippingAddress.name, data.shippingAddress.line1, data.shippingAddress.line2, `${data.shippingAddress.city||''} ${data.shippingAddress.region||data.shippingAddress.state||''} ${data.shippingAddress.postal_code||''}`, data.shippingAddress.country].filter(Boolean).join('\n')}</div>
          ) : (<div className="text-sm text-muted-foreground">—</div>)}
        </div>
      </div>
            ...
            <tbody>
              {data.items.map((it:any)=>(
                <tr key={it.id} className="border-b last:border-0">
                  ...
                  <td className="py-2 pr-4 text-right">{Number(it.quantity || it.qty_ordered || 0)}</td>
                  <td className="py-2 pr-4 text-right">{(it.total_price ?? it.amount)!=null? `${o.currency||'USD'} ${Number(it.total_price ?? it.amount).toFixed(2)}` : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              <tbody>
                {data.transactions.map((t:any)=>(
                  <tr key={t.id} className="border-b last:border-0">
                    <td className="py-2 pr-4 whitespace-nowrap">{t.created_at? new Date(t.created_at).toLocaleString() : '—'}</td>
