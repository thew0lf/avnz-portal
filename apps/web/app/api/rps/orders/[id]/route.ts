...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  let r = await fetch(`${aiBase}/rps/orders/${encodeURIComponent(params.id)}`, { headers: { authorization: `Bearer ${token}` }, cache: 'no-store' })
  if (r.status === 401) {
    ...
    if (refresh) {
      ...
      if (rr.ok) {
        ...
        r = await fetch(`${aiBase}/rps/orders/${encodeURIComponent(params.id)}`, { headers: { authorization: `Bearer ${newToken}` }, cache: 'no-store' })
      }
    }
  }
