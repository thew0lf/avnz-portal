...
export async function GET(req: NextRequest) {
  ...
  let r = await fetch(`${aiBase}/rps/transactions${search}`, { headers: { authorization: `Bearer ${token}` }, cache: 'no-store' });
  if (r.status === 401) {
    ...
    if (refresh) {
      ...
      if (rr.ok) {
        ...
        r = await fetch(`${aiBase}/rps/transactions${search}`, { headers: { authorization: `Bearer ${newToken}` }, cache: 'no-store' });
      }
    }
  }
