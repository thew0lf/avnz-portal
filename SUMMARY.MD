...
## Repo Hygiene (AVNZ-10)
- Updated `.gitignore` to explicitly ignore nested `apps/*/node_modules`, `.next`, `dist`, `build`, `coverage`, and local markers (e.g., `.away_mode`).
- Removed accidentally tracked artifacts from Git history using `git filter-branch`:
  - Branches cleaned: `work/design-2/AVNZ-10`, `design-2`, `main` (and `origin/main`).
  - Paths purged across history: `apps/*/node_modules`, `apps/*/.next`, `.idea`, `.env`, plus explicit large files that triggered failures.
  - Force-pushed rewritten branches; remote now free of >100MB blobs (Next.js SWC) and large Next cache pack files.
- No application code changes; only VCS hygiene. Local `.env`/IDE files remain untracked. Future commits inherit the stricter ignore rules.
 - Added hooks:
   - `pre-commit` blocks committing `node_modules/`, `.next/` and files > 50MB, and runs lint.
   - `pre-push` blocks pushing if `HEAD` contains disallowed paths or files > 50MB.
...
Update 2025-09-28 (Git workflow)
- Worker PR base: default PR base branch set to `design-2` via new env `GIT_PR_BASE` (worker uses this when opening PRs). Code: `apps/ai/app/assistants/worker.py`.
- Branch namespace: standardized to `work/design-2/<EPIC>/<TICKET>` (avoids branch-vs-directory collision with `design-2`). Env: `GIT_NS_PREFIX=work`, `GIT_WORK_BRANCH=design-2`.
- Compose: added `GIT_PR_BASE` env to `aiworker` service in `docker-compose.yml`.
- Docs updated: `README.md` (Branching & PRs), `JIRA.md` (namespace + env), this `SUMMARY.MD` entry.
- Env expectations: set in `.env` or compose
  - `AUTO_COMMIT=1`
  - `GIT_REMOTE_URL=https://$GITHUB_TOKEN@github.com/thew0lf/avnz-portal.git`
  - `GITHUB_REPO=thew0lf/avnz-portal`
  - `GIT_PR_BASE=design-2`
  - `GIT_WORK_BRANCH=design-2`, `GIT_NS_PREFIX=work`
  - restart worker: `docker compose up -d --build aiworker`

Update 2025-09-28 (Repo hygiene)
- Stop tracking Next.js build artifacts under `apps/web/.next`; added recursive ignore for monorepo (`**/.next/`, `**/.next/cache/`).
- Files touched: `.gitignore`; removed tracked `.next` files from history on branch `work/design-2/AVNZ-10`.
